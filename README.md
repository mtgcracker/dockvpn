...
```bash
CID=$(docker run -d --privileged -p 1194:1194/udp -p 443:443/tcp mtgcracker/dockvpn)
docker run -t -i -p 8080:8080 --volumes-from $CID mtgcracker/dockvpn serveconfig
```
...
When the `mtgcracker/dockvpn` image is started, it generates:
...
Conveniently, `mtgcracker/dockvpn` comes with a script called `serveconfig`,
