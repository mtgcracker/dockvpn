...
```bash
CID=$(docker run -d --privileged -p 1194:1194/udp -p 443:443/tcp mgraham/dockvpn)
docker run -t -i -p 8080:8080 --volumes-from $CID mgraham/dockvpn serveconfig
```
...
When the `mgraham/dockvpn` image is started, it generates:
...
Conveniently, `mgraham/dockvpn` comes with a script called `serveconfig`,
